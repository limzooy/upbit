# test1_1 매도 조건: 볼린저 상단 and rsi 65이상
...
#             for j in range(1, 11):  # 5개의 거미줄 주문 생성
...
# def main():
#     # CSV 파일 로드
#     df = pd.read_csv('C:\\Users\\limjy\\upbit\\proj\\upbit_candle_data(1).csv')
...
#     trade_df.to_csv('trade_backtest_v1_1(1).csv', index=False)
#     print("Trade history saved to 'trade_backtest_v1_1(1).csv'")
    ...
    for i in range(len(df)):
        ...
        if not is_buy_web_active and not is_sell_web_active and balance > order_amount and current_price < lower_band and rsi < 40:
            ...
            for j in range(1, 11):  # 10개의 거미줄 주문 생성
                order_price = current_price * (1 - buy_interval * j)
                buy_orders.append({'price': order_price, 'amount': order_amount})
        ...
        if not is_sell_web_active and not is_buy_web_active and btc > 0 and current_price > upper_band and rsi > 65:
            ...
            total_sell_amount = btc * 0.5  # 보유 코인의 50%
            for j in range(1, 11):  # 10개의 거미줄 주문 생성
                order_price = current_price * (1 + sell_interval * j)
                sell_amount = total_sell_amount / 5  # 각 주문당 총 매도량의 1/5
                sell_orders.append({'price': order_price, 'amount': sell_amount})
...
def main():
    ...
    # 거래 기록을 DataFrame으로 변환하여 CSV로 저장
    trade_df = pd.DataFrame(trade_history, columns=['Type', 'Price', 'Amount', 'Balance', 'BTC'])
    trade_df.to_csv('trade_backtest_v1_1(4).csv', index=False)
    print("Trade history saved to 'trade_backtest_v1_1(4).csv'")
